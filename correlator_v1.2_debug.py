...
def is_ion(peaks_triplet, ion='CIV', tolerance= 5e-3):
    ...
    if(np.all(model_data_difference <= tolerance)):
        return True
    else:
        return False


tol_plot = Table(names=['number', 'tolerance'])

for j in np.arange(0.1, 200, 0.01):
    print("\n Tolerance:", j*1e-3)
    cor_peaks = []
    for i in range(len(peaks)):
        if(i == 0 or i == len(peaks) - 1):
            ...
            if(is_ion(peaks_triplet, ion='CIV', tolerance= j*1e-3)):
                #print("Possible system:", peaks_triplet[1])
                cor_peaks.append(peaks_triplet[1])

    if(len(cor_peaks) > 0):
        tol_plot.add_row([len(cor_peaks), j*1e-3])
    else:
        tol_plot.add_row([0, j*1e-3])


plt.plot(tol_plot['tolerance'], tol_plot['number'])
plt.xlabel('Tolerance')
plt.ylabel('Number of peaks')
plt.title('Number of peaks vs Tolerance')
plt.show()
