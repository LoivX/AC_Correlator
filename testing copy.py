...
if __name__ == '__main__':  
    ...
    for snr in [50, 100, 200]:
        ...
        for i in range(5,15,1):
            ...
            threshold = correlator(, resol, logN +0.1*i, b, ion, dz)

            for j in range(1,10,1):
                spectrum_file = rf'spectra\test_{i:02d}_{snr}_{j}_spec.dat'
                print(f'logN = {logN + 0.1*i},  iteration = {j}')
                print('Processing file:', spectrum_file)
                cor_final, z_interval, peaks = correlator(spectrum_file, resol, logN +0.1*i, b, ion, dz)
                recall[j-1], precision[j-1], f1[j-1] = RPF1(peaks, synthetic_systems[j], b)
                plot_correlation(cor_final, z_interval, peaks, synthetic_systems[j], logN +0.1*i, threshold, snr, j, rf'D:\Università\terzo anno\Tesi\Immagini\v3_final\{snr}\{i:02d}')
                print('\n')
